...
class LoRA2GLUETrainer(GeneralTorchTrainer):
        ...
        if ctx.cfg.llm.deepspeed.use:
            ...
            if ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
                ...
                if ctx.cfg.federate.alternate_training:
                    ...
                    logger.info("LoRA-A2: not alternate training")
    ...
    def get_model_para(self):
        all_params = super().get_model_para()
        if self.cfg.federate.alternate_training or self.cfg.federate.alternate_communication:
            result = {}
            for name, param in all_params.items():
                if "lora" in name:
                    if self.ctx.round % 2 == 0 and "lora_A" in name:
                        result[name] = param
                    if self.ctx.round % 2 == 1 and "lora_B" in name:
                        result[name] = param
                else:
                    result[name] = param
            return result
